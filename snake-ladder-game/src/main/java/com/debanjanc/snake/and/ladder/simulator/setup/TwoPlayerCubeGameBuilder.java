...
public class TwoPlayerCubeGameBuilder extends GameBuilder<TwoPlayerCubeGame> {
	...
	private Board buildBoard() {
		...
		/* Initialize */
		for (int i = 0; i < gameLoader.getBoardSize() + 1; ++i) {
		...
		for (Ladder ladder : ladders) {
			cells.get(ladder.getStart() - 1).setEffectiveCell(cells.get(ladder.getEnd() - 1));
		}

		/* setup snakes */
		List<Snake> snakes = gameLoader.getSnakes();
		for (Snake snake : snakes) {
			cells.get(snake.getHead() - 1).setEffectiveCell(cells.get(snake.getTail() - 1));
		}
